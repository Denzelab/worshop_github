...
static int read_memory_value(corewar_t *cw, int address)
{
    int result = 0;
...
void exec_ldi(corewar_t *cw, process_t *proc)
{
    ...
    int address = (proc->pc + ((param1 + param2) % IDX_MOD)) % MEM_SIZE;

    if (reg_num < 1 || reg_num > REG_NUMBER) {
        proc->pc = (proc->pc + 7) % MEM_SIZE;
        return;
    }
    proc->registers[reg_num - 1] = read_memory_value(cw, address);
    proc->carry = (proc->registers[reg_num - 1] == 0) ? 1 : 0;
    proc->pc = (proc->pc + 7) % MEM_SIZE;
    proc->cycles_left = op_tab[9].nbr_cycles - 1;
}
